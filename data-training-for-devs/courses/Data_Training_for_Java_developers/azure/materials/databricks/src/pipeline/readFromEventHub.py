...
# MAGIC %md 
# MAGIC ## 2. Import properties
# MAGIC - Import *observation* schema from `pipeline\schema\observation.py` by using magic (run) command.
# MAGIC - Import `pipeline\properties.py` by using magic (run) command.

# COMMAND ----------

# MAGIC %md 
# MAGIC ## 3. Read event hub connection string value from secrets. 
# MAGIC It has been already added during installation process. 
# MAGIC Use `dbutils.secrets.get()` method.
...
# MAGIC - To retrieve payload from the binary data use the following statement:
...
# MAGIC %md 
# MAGIC ## 7. Write data to the newly created table.
